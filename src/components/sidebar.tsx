...
import { LogOut, Settings2, Sparkles, Pin, X } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col scroll-smooth px-3 pt-4">
          <nav className="flex flex-col gap-1.5 w-full">
            ...
            <motion.div
              ...
              transition={SIDEBAR_SPRING}
              className="flex items-center h-9 gap-3"
            >
              <div className="w-9 h-9 -ml-2.5 flex items-center justify-center shrink-0 rounded-lg">
                <DomainIcon
                  size={16}
              ...
              <motion.span
                ...
                transition={LABEL_TRANSITION}
                className="text-micro font-mono uppercase tracking-wider whitespace-nowrap -ml-1"
                style={{ color: color.text, pointerEvents: isExpanded ? "auto" : "none" }}
              >
                {domain.label} Space
              </motion.span>
            </motion.div>

            <div className="h-px mb-1" style={{ backgroundColor: "rgba(255,255,255,0.04)" }} />
            ...
            {domain.spaces.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-caption transition-colors duration-200 relative ${
                    isActive
                      ? "font-medium text-text-primary"
                      : "text-text-secondary hover:text-text-primary hover:bg-surface-hover"
                  }`}
                  style={{ backgroundColor: isActive ? color.bgActive : undefined }}
                >
                  {isActive && (
                    <div
                  ...
                  <Icon
                    ...
                    className="shrink-0"
                  />
                  <span className="truncate" style={{ color: isActive ? color.text : undefined }}>
                    {item.label}
                  </span>
                </Link>
              );
