        ...
        {items.map((item) => (
          <div key={item.label} className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-caption font-mono text-muted uppercase tracking-[0.2em]">{item.label}</p>
            </div>
            <p className="text-body font-heading text-text uppercase tracking-tight">{item.value}</p>
          </div>
        ))}
      </div>
    </div>
