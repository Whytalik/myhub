  ...
  return (
    <div className="bg-surface/50 border border-red-500/20 rounded-2xl p-10 flex flex-col items-center text-center gap-6 max-w-lg mx-auto">
      ...
      <button
        ...
        className="bg-accent text-bg px-8 py-3 rounded-xl text-note font-mono uppercase tracking-[0.2em] hover:scale-105 transition-all"
      >
        Refresh Page
      </button>
      ...
      {developerError && (
        <div className="w-full mt-4">
          <button
            ...
            className="flex items-center gap-2 mx-auto text-caption font-mono text-muted hover:text-text transition-colors uppercase tracking-[0.2em]"
          >
            <ChevronDown
              size={12}
          ...
          {showDevLog && (
            <div className="mt-4 bg-bg border border-border rounded-xl p-4 text-left">
              <div className="flex items-center justify-between mb-3">
                <span className="text-caption font-mono text-red-400 uppercase tracking-widest">
                  Error Details
                </span>
                <button
                  onClick={() => navigator.clipboard.writeText(developerError)}
                  className="flex items-center gap-1.5 text-caption font-mono text-muted hover:text-text transition-colors"
                >
                  <Copy size={11} />
                  <span>Copy</span>
                </button>
              </div>
              <pre className="text-note font-mono text-secondary whitespace-pre-wrap break-all max-h-48 overflow-y-auto">
                {developerError}
              </pre>
            </div>
          )}
