...
import { TRAINING_ACCENT_SECONDARY, CHART_GRIDLINE } from "./chart-tokens";

interface RirTrendChartProps {
  points: { date: Date; avgRir: number }[];
}
...
export function RirTrendChart({ points }: RirTrendChartProps) {
  ...
  const coords = useMemo(
    () =>
      points.map((p, i) => ({
        x: PAD_LEFT + i * step,
        y:
          PAD_TOP + plotH * (1 - (Math.min(Math.max(p.avgRir, yMin), yMax) - yMin) / (yMax - yMin)),
      })),
    [points, step, plotH],
  );
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Тренд середнього RIR по сесіях"
    >
      {gridValues.map((v) => {
      ...
      {areaPath && <path d={areaPath} fill={TRAINING_ACCENT_SECONDARY} opacity={0.1} />}
      <path
        d={linePath}
        fill="none"
        stroke={TRAINING_ACCENT_SECONDARY}
        strokeWidth={2}
        strokeLinecap="round"
      ...
      {coords.length > 0 && (
        <>
          <circle
            ...
            r={4}
            fill={TRAINING_ACCENT_SECONDARY}
            stroke="#1c1c1e"
            strokeWidth={2}
          />
          <text
          ...
          >
            {points[points.length - 1].avgRir.toFixed(1)}
          </text>
        </>
      )}
      ...
      {hovered && (
        <g pointerEvents="none">
          ...
          >
            RIR {hovered.point.avgRir.toFixed(1)}
          </text>
        </g>
      )}
