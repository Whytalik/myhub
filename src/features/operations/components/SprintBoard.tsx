  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <span className="text-caption font-mono text-muted uppercase tracking-wider mb-1 block">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="px-3 py-1 bg-accent/10 border border-accent/20 rounded-lg">
               <span className="text-caption font-mono text-accent uppercase font-medium tracking-wider">
        ...
        <div className="flex items-center gap-3">
           <div className="px-4 py-2 bg-success/10 border border-success/20 rounded-lg">
              <span className="text-caption font-mono text-success uppercase font-medium tracking-wider">
                Active Cycle
              </span>
           </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                     ...
                     <div className="space-y-3">
                       {sprint.objectives.flatMap(obj => obj.projects).length === 0 ? (
                         ...
                         sprint.objectives.flatMap(obj => obj.projects).map(project => (
                           <div key={project.id} className="p-4 bg-surface rounded-xl">
                             <div className="flex justify-between items-start mb-2">
                                <span className="text-caption font-medium uppercase text-text truncate pr-2">
                                  {project.title}
                                </span>
                   ...
                   <div className="p-6 bg-surface rounded-xl">
                      <h4 className="text-caption font-medium uppercase tracking-wider mb-4 text-text">Weekly Execution Score</h4>
                      <div className="flex items-center gap-4">
                         ...
                         <div className="flex-1">
                            <div className="h-2 w-full bg-border/30 rounded-full overflow-hidden">
                               <div className="h-full bg-success" style={{ width: `${calculateExecutionScore(sprint, currentWeek)}%` }} />
                            </div>
...
function ObjectiveCard({ objective, onRefresh, currentWeek }: { objective: ObjectiveData, onRefresh: () => void, currentWeek: number }) {
  return (
    <div className="bg-surface rounded-xl overflow-hidden group">
      <div className="p-6 border-b border-border/50">
        ...
        <div className="space-y-4">
           {objective.keyResults.map(kr => (
             <div key={kr.id} className="space-y-4 p-4 bg-surface-hover rounded-xl">
           ...
           {objective.keyResults.length < 3 && (
             <KeyResultDialog objectiveId={objective.id} onSuccess={onRefresh}>
               <Button variant="ghost" size="sm" className="w-full border border-dashed border-border h-8 text-caption uppercase tracking-wider text-muted hover:text-accent hover:border-accent/40">
      ...
      <div className="px-6 py-3 bg-surface-hover flex justify-between items-center">
         ...
         <ProjectDialog objectiveId={objective.id} onSuccess={onRefresh}>
            <Button variant="ghost" size="sm" className="h-7 px-2 text-caption uppercase font-medium text-accent">
