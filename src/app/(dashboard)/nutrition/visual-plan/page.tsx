...
const mealCategories = [
  ...
  },
  {
    ...
    variants: [
      { name: "Сирні панкейки", icon: "🥞", products: ["Сир кисломолочний", "Яйце", "ЦЗ Борошно", "Яблуко / Груша / Вишня замор. / Чорниця замор.", "Мед", "Кориця", "Ванілін"] },
      { name: "Білковий омлет", icon: "🍳", products: ["Яйця", "Яєчні білки", "Куряче філе", "Помідор / Мікс овочей", "ЦЗ Хліб", "Насіння льону", "🧂 Соєво-часниковий"] },
      { name: "Тости з тунцем", icon: "🐟", products: ["Тунець у вл. соку", "ЦЗ Хліб", "Яйце варене", "Огірок / Зелена цибуля", "Насіння льону", "🥣 Гірчиця"] },
      { name: "Breakfast Wrap", icon: "🌯", products: ["Лаваш тонкий", "Куряче філе", "Яйце-скрембл", "Айсберг / Огірок", "Насіння льону", "Помідор", "Цибуля червона", "Лимонний сік"] },
    ]
  },
  {
    title: "🥗 Обід (13:30)",
    ...
    variants: [
      { name: "Зібраний Боул", icon: "🥣", products: ["Гречка / Рис / Булгур", "Смажений фарш / Курячі серця", "Морква", "Огірок", "Помідор", "Яблуко / Груша", "Олія оливкова / Арахіс смажений", "🥣 Соєво-медовий"] },
      { name: "Паста-конструктор", icon: "🍝", products: ["Макарони ТЗ", "Куряче філе / Яловичий фарш", "Томатна пасата", "Цибуля", "Морква", "Твердий сир", "🥣 Орегано + Базилік"] },
      { name: "Зігріваючий Суп", icon: "🍲", products: ["Червона сочевиця", "Куряче філе", "Картопля", "Морква", "Цибуля", "Петрушка / Кріп", "Куркума", "Лавровий лист"] },
      { name: "Печена тарілка", icon: "🐟", products: ["Картопля", "Хек / Тилапія / Куряче стегно", "Морква по-корейськи / Капуста", "🧂 Лимонно-трав'яний", "🥣 Ткемалі"] },
    ]
  },
  {
    title: "🌙 Вечеря (19:30)",
    ...
    variants: [
      { name: "Овочеве Рагу", icon: "🍛", products: ["Куряче філе / Яловичий фарш", "Картопля", "Морква", "Кабачок", "Перець", "Квасоля", "Томатна паста", "🧂 Паприка + Лавровий лист"] },
      { name: "Солодкий Сир", icon: "🍓", products: ["Сир кисломолочний", "Яблуко / Груша / Вишня замор. / Чорниця замор.", "Арахіс / Насіння льону", "Мед", "Кориця"] },
      { name: "Риба + Зелень", icon: "🥦", products: ["Хек / Тилапія / Мінтай", "Гречка / Рис", "Броколі зам. / Цвітна капуста зам.", "🧂 Олія + Кріп + Часник", "Грецький йогурт"] },
      { name: "Протеїновий Салат", icon: "🥗", products: ["Тунець / Курка / Яйця", "Айсберг / Пекінська капуста (200г+)", "Огірок / Помідор", "🥣 Йогурт з кропом"] },
...
const fastFoodOptions = [
  {
    ...
    items: [
      { who: "both", name: "Класичний рол", ing: ["Норі", "Рис суші", "Лосось / Форель", "Плавлений сир", "Огірок"] }
    ]
  },
  {
...
const marinades = [
  ...
  { name: "Паприка + Лавровий лист", used: ["Рагу"], ing: ["Олія — 2 ст.л.", "Паприка — 1 ч.л.", "Лавровий лист — 2 шт", "Перець — ½ ч.л."] },
  { name: "Лимонно-трав'яний", used: ["Печена тарілка"], ing: ["Лимонний сік — 2 ст.л.", "Орегано — 1 ч.л.", "Олія — 1 ст.л."] },
];
...
const sauces = [
  ...
  { name: "Соєво-медовий", used: ["Боул"], ing: ["Соєвий соус — 2 ст.л.", "Мед — ½ ч.л.", "Часник — 1 зуб."] },
  { name: "Ткемалі", used: ["Печена тарілка"], ing: ["Ткемалі готовий — 2 ч.л."] },
  { name: "Йогурт з кропом", used: ["Протеїновий салат"], ing: ["Грецький йогурт — 2 ст.л.", "Кріп — 1 ст.л."] },
];
                      ...
                      {cat.items.map((item, iIdx) => (
                        <div key={iIdx} className="flex flex-col gap-1">
                          <table className="w-full text-left border-separate border-spacing-y-0.5">
                            <thead>
                              <tr className="text-[6px] uppercase text-[#f7a948]/40 tracking-widest">
                                <th className="font-normal pb-0.5">Склад ({item.name})</th>
                                <th className="font-normal pb-0.5 text-right w-4">В</th>
                                <th className="font-normal pb-0.5 text-right w-4">О</th>
                              </tr>
                            </thead>
                            <tbody>
                              {item.ing.map((ing, ingIdx) => (
                                <tr key={ingIdx}>
                                  <td className="text-[9px] text-muted leading-tight py-0.5 flex items-start gap-1">
                                    <span className="w-1 h-1 rounded-full bg-[#f7a948]/30 mt-1 shrink-0" />
                                    {ing}
                                  </td>
                                  <td className="text-[8px] text-gray-500 text-right align-top py-0.5">
                                    {(item.who === "you" || item.who === "both") ? "1п" : "—"}
                                  </td>
                                  <td className="text-[8px] text-gray-500 text-right align-top py-0.5">
                                    {(item.who === "her" || item.who === "both") ? "1п" : "—"}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
