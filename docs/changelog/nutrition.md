## [2026-06-02] — Feat: Алгоритм точної підгонки макросів під норми (Lagrange fitter)

Реалізовано математично точний алгоритм підгонки ваг інгредієнтів під денні макро-цілі кожної людини. Використовує метод Лагранжа (мінімізація зміни ваг за квадратичним критерієм з обмеженнями: Б, Ж, В = цільові значення). Три файли:

- **`src/lib/nutrition/macro-fitter.ts`** — чиста функція `fitDayMacros(ingredients, targets)`: будує матрицю Грама з поточних ваг, розв'язує систему 3×3 методом Гауса, знаходить масштаб-фактори `sᵢ = 1 − μ·(macroᵢ·wᵢ)` і округлює до грама.
- **`fitDayToNormsAction(dayPlanId, personId)`** — серверна дія в `planning.ts`: завантажує всі незаблоковані слоти, враховує `inputState` (RAW/COOKED) і коефіцієнт приготування, запускає fitter, оновлює усі ваги в транзакції.
- **`WeekPlanner.tsx`** — кнопка "Підігнати під норми" під кожним блоком MacroSummary (per person, per day).

**Результат**: після натискання кнопки Б/Ж/В збігаються з цільовими значеннями з точністю ±1–2 г (похибка від округлення до цілого грама).

- **Верифікація**:
    - [x] Логіка реалізована
    - [x] UI оновлено
    - [x] Перевірено `pnpm tsc --noEmit` — 0 помилок
